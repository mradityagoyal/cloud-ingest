...
type MessageHandler interface {
	// HandleMessage processes a TaskUpdate constrcuted from Pub/Sub task progress
	// message. It should fill in the new tasks generated by this task update in
	// the taskUpdate object.
	HandleMessage(jobSpec *JobSpec, taskUpdate *TaskUpdate) error
}
...
func (r *MessageReceiver) ReceiveMessages() error {
	...
	err := r.Sub.Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
		...
		taskUpdate, err := TaskCompletionMessageJsonToTaskUpdate(decodedMsg)
		if err != nil {
		...
		if err := r.Handler.HandleMessage(jobSpec, taskUpdate); err != nil {
			log.Printf(
