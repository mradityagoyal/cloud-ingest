...
func checkDownloadTaskSpec(d *taskpb.DownloadSpec) (resumedCopy bool, err error) {
	if d.SrcObject == "" {
		return false, errors.New("empty SrcObject")
	} else if d.SrcBucket == "" {
		return false, errors.New("empty SrcBucket")
	} else if d.DstFile == "" {
		return false, errors.New("empty DstFile")
	} else if d.GenerationNum < 0 {
		return false, fmt.Errorf("invalid GenerationNum: %v", d.GenerationNum)
	} else if d.BytesCopied < 0 {
		return false, fmt.Errorf("BytesCopied <= 0: %v", d.BytesCopied)
	}

	if d.BytesCopied > 0 {
		if d.FileBytes <= 0 {
			return true, fmt.Errorf("resumedCopy but FileBytes <= 0: %v", d.FileBytes)
		}
		return true, nil
	}
