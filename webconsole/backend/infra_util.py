...
def _pubsub_status_from_bldr(pubsub_bldr):
    """Gets the ingest Pub/Sub deployment status. It uses the passed builder
    objects to check for resources statues.
    ...
    Returns:
        A dictionary that contains all pub/sub topics and subscriptions
...
def infrastructure_status(credentials, project_id):
    ...
    return _pubsub_status_from_bldr(pubsub_bldr)
...
def create_infrastructure(credentials, project_id):
    ...
    """
    # Creating the pubsub builder.
    pubsub_bldr = pubsub_builder.PubSubBuilder(
        credentials=credentials, project_id=project_id)

    # Checking the infrastructure deployment status before creating it.
    pubsub_statuses = _pubsub_status_from_bldr(pubsub_bldr)
    # Make sure all infrastructure components are not found.
    if not dict_values_are_recursively(pubsub_statuses,
...
def tear_infrastructure(credentials, project_id):
    ...
    if dict_has_values_recursively(pubsub_statuses,
