...
class SpannerWrapper(object):
  ...
  def get_job_config(self, config_id):
    ...
    Returns:
      A dictionary containing the desired job config, mapping from attribute
      to value.
    """
  ...
  def create_job_run(self, config_id, run_id):
    ...
    return self.insert(
        ...
        SpannerWrapper.JOB_RUNS_COLUMNS,
        values
    )
  ...
  def get_job_runs(self):
    ...
    """
    # TODO(b/64076262): Add pagination to job runs since there may be many runs
  ...
  def get_tasks_for_run(self, config_id, run_id, max_num_tasks=25,
    ...
    Args:
      ...
      max_num_tasks: The number of tasks to return, default is 25. Must be >= 0.
    ...
    params = {"run_id": run_id, "config_id": config_id,
    ...
    if task_type:
      query += (" AND STARTS_WITH(%s, @task_type)" % SpannerWrapper.TASK_ID)
  ...
  def single_result_query(self, query, query_params=None, param_types=None):
    ...
    Returns:
      A dictionary mapping from attribute name to value, or None if the query
      had no results.
    """
    results = self.database.execute_sql(query, query_params, param_types)
  ...
  def row_to_object(self, result, fields):
    """Processes a single result of a StreamedResultSet and returns the result.
...
if __name__ == "__main__":
  # Code for testing the class locally
  if len(sys.argv) != 4:
    print ("Usage: python spannerwrapper.py <instance id> <database name> "
  ...
  database = sys.argv[2]
  path = sys.argv[3]
  spanner_client = SpannerWrapper(path, instance, database)
